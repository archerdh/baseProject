...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        SEL originalSel = @selector(scrollpage_viewDidAppear:);
        SEL newSel = @selector(viewDidAppear:);
...
- (void)layoutPageViewController {
    ...
    self.pageViewController.view.frame = CGRectMake(0, originY, self.view.frame.size.width, kMainBoundsHeight - 50 - KTC_TOP_MARGIN - 44);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (self.inDragging) {
        ...
        CGFloat offsetX = ABS(offset.x - kMainBoundsWidth);
        if (offsetX > kMainBoundsWidth / 2) {
            self.currentIndex = self.nextIndex;
        }
        CGFloat progress = offsetX / kMainBoundsWidth;
